...
class Web_model {
	...
	public function hitung_total_pembayaran() {

        $jumlah_bayar = $this->input->post('jumlah_bayar', TRUE);
        $jumlah_tanggungan = $this->input->post('jumlah_tanggungan', TRUE);
        $total_pembayaran = $jumlah_bayar * $jumlah_tanggungan; 

        return $total_pembayaran;

    }
	function calculate_total_payment($data) {

        $jenis_pembayaran = $data['jenis_pembayaran'];
        $jumlah_tanggungan = $data['jumlah_tanggungan'];


        if ($jenis_pembayaran == 'Pembayaran Uang') {
            $value = 50000;
        } elseif ($jenis_pembayaran == 'Pembayaran Beras') {
            $value = 2.5;
        } else {
            $value = 0;
        }

        $total_pembayaran = $value * $jumlah_tanggungan;
